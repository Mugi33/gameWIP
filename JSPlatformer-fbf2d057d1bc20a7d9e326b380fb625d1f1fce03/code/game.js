...
function Flag(pos){
  this.basePos = this.pos = pos.plus(new Vector(0.1,0.0));
  this.size = new Vector(1.0, 1.0);
  this.shake = Math.random() * Math.PI * 4;
}
Flag.prototype.type = "flag";

function Goblin(pos){
  this.basePos = this.pos = pos.plus(new Vector(0.1,-.5));
  this.size = new Vector(2.0, 2.0);
  this.gob = Math.random() * Math.PI * 4;
}
Goblin.prototype.type = "goblin";
...
Flag.prototype.act = function(step) {
  ...
  this.pos = this.basePos.plus(new Vector (shakePos, -0.5));
};

var gobSpeed = 2
var gobDist = .7
Goblin.prototype.act = function(step) {
  this.gob += step * gobSpeed;
  var gobPos = Math.sin(this.gob) * gobDist;
  this.pos = this.basePos.plus(new Vector (gobPos, -0.5));
};
...
if (type == "goblin" && this.status == null) {
  this.status = "lost";
  this.finishDelay = 1;
}
}
